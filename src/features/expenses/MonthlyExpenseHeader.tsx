  ...
  return (
    <div className="bg-gradient-primary text-primary-foreground rounded-xl p-3 shadow-elevated">
      ...
      <div className="text-center mt-0.5">
        <div className="font-black text-2xl tabular-nums leading-tight">{fmtMoney(total)}</div>
        <div className="text-[10px] opacity-80 mt-0.5">{baseName}</div>
      </div>
      {budget > 0 && (
        <div className="mt-2 bg-white/10 rounded-lg p-2">
          ...
          {over && <div className="text-[10px] mt-1 opacity-95">⚠️ تجاوزت الميزانية بـ {fmtMoney(total - budget)}</div>}
          {!over && pct > 80 && <div className="text-[10px] mt-1 opacity-95">⚡ اقتربت من حد الميزانية</div>}
        </div>
      )}
    </div>
  );
